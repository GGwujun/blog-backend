...
![图片](./httpsstatic001geekbangorgresourceimage3e1d3e410fb54d3b69358ca72ffc321dcd1d.jpg "Spark SQL与Spark Core的关系")
...
![图片](./httpsstatic001geekbangorgresourceimagecbfacbfdebe214a4d0f89ff1f4704e5913fa.jpg "Catalyst优化器与Tungsten")
...
![图片](./httpsstatic001geekbangorgresourceimage73cb73b8688bbd3564f30e856d9df46a8ccb.jpg "“倍率分析”的AST语法树 / 执行计划")
...
![图片](./httpsstatic001geekbangorgresourceimage72e472781191ddf37608602cdb0690c0e9e4.jpg "谓词下推与列剪枝示意图")
...
![图片](./httpsstatic001geekbangorgresourceimage57dd57029cabc2155c72ddbffb6c8ab440dd.jpg "经过逻辑优化的执行计划")
...
![图片](./httpsstatic001geekbangorgresourceimage242524675c8d5e31c51e7yyd6336acf3f525.jpg "数据表的Data Schema")

但如果用Tungsten Unsafe Row数据结构进行存储的话，每条数据记录仅需消耗十几个字节，如下图所示。

![图片](./httpsstatic001geekbangorgresourceimage6ec76eb33b3yy4b4cd658e9739b8a75321c7.jpg "使用Unsafe Row来存储数据记录")

说完了Unsafe Row的数据结构优化，接下来，我们再来说说WSCG：全阶段代码生成。所谓全阶段，其实就是我们在调度系统中学过的Stage。以图中的执行计划为例，标记为绿色的3个节点，在任务调度的时候，会被划分到同一个Stage。

![图片](./httpsstatic001geekbangorgresourceimage57dd57029cabc2155c72ddbffb6c8ab440dd.jpg "绿色节点同属于一个Stage")
...
首先，在RDD开发框架下，Spark Core的优化空间受限。绝大多数RDD高阶算子所封装的~~封装的~~计算逻辑（形参函数f）对于Spark Core是透明的，Spark Core除了用闭包的方式把函数f分发到Executors以外，没什么优化余地。
...
![图片](./httpsstatic001geekbangorgresourceimagecbfacbfdebe214a4d0f89ff1f4704e5913fa.jpg)
